...
namespace search {
  ...
  namespace detail {
    ...
    auto boyer_moore_horspool_search(const std::string_view& needle,
				     const std::string_view::const_iterator& haystack_begin,
				     const std::string_view::const_iterator& haystack_end) {
    ...
    auto print_colored(std::string_view str, std::string_view query, bool ignore_case)
    {
  ...
  void file_search(fs::directory_entry const& dir_entry, std::string_view query, bool ignore_case, bool show_line_numbers)
  {
    ...
    if (ignore_case) {
      ...
      auto ptr = detail::boyer_moore_horspool_search(query, buffer.begin(), buffer.end());
      it = ptr - buffer.begin();
	...
	auto ptr = detail::boyer_moore_horspool_search(query, &buffer[line_end], buffer.end());
	it = ptr - &buffer[line_end];
