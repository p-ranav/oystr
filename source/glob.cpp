    ...
    if (c == '*') {
      ...
      if (j >= n) {
        ...
        if (stuff.find("--") == std::string::npos) {
          ...
          while (true) {
            ...
            chunks.push_back(
                std::string(pattern.begin() + i, pattern.begin() + k));
            i = k + 1;
            k = k + 3;
          }
      ...
      if (special_characters_map.empty()) {
        for (auto& c : special_characters) {
          special_characters_map.insert(std::make_pair(
              static_cast<int>(c), std::string {"\\"} + std::string(1, c)));
        }
      }
...
// directory.
std::vector<fs::path> glob2(const fs::path& dirname,
...
std::vector<fs::path> glob1(const fs::path& dirname,
                            const fs::path& pattern,
                            bool dironly)
{
  // std::cout << "In glob1\n";
...
std::vector<fs::path> glob0(const fs::path& dirname,
...
std::vector<std::filesystem::path> rglob(
    const std::vector<std::string>& pathnames)
{
  std::vector<std::filesystem::path> result;
  for (auto& pathname : pathnames) {
